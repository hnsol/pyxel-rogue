...
GET成功時のonline rankingは保存し、次回Online Ranking入場時は通信なしでも保存済みrankingとlocal bestを合成表示します。

オンラインランキングは Google Sheets + Apps Script 連携用の雛形を `docs/apps_script_scoreboard.gs` に置いています。公開先を差し替える場合は `PYXEL_ROGUE_SCORE_URL` を指定してください。ゲーム終了時はローカル保存だけを行い、Online Ranking では My Rogue Chronicle を即表示します。Title から入ってもゲームオーバー後に入っても入場時は通信せず、保存済みランキングとlocal bestを合成表示します。Logo後、オンラインprofile未作成なら登録確認を出します。オンライン名は小文字英数字のみで、初期値は `rogue54` です。未登録・未送信保証の名前はゲーム内で `rogue54*` のように `*` を付け、正式な保存名・送信名には `*` を含めません。登録/切替は Title の Name または Online Ranking から行い、serverの user list を `checkUser` で確認します。既存名は本人確認として6桁PINでリンクし、新規名は6桁PINと server token を登録します。Title の Name で登録済み名を編集してBを押した場合は、確認後にlocal-only名として保存してTitleへ戻ります。server token はローカルで難読化保存し、POST時に照合します。UI文言では `sync` / `synced` を避け、ローカル保存は saved、サーバ送信は posted、ランキングGET更新は refreshed と呼び分けます。通信は登録/リンク直後と Select から行い、POSTは24時間に1回だけ server時刻の `last_sync_at` / `next_sync_at` で制限します。24時間以内にSelectした場合はPOSTせずGETだけを行い、完了結果は右上、現在状態は19行目の `POST after` で表示します。POSTはlocal best投稿とprofile時刻更新だけを扱い、Weekly / Season はGET時に取得します。スコアボードはトップ10のみを表示します。Apps Script の users sheet は `user_password` を文字列列として扱います。新しいweek/seasonで空にならないよう、ランキングGET時に必要なdummy行をlazy生成し、`score_id` で重複行を抑止します。GET時のscore sheet読み込みは1回のcontextへまとめ、生成dummy行は一括で書き込みます。旧オンラインprofileは開発中リセットとして v3 storage へ切り替えています。
