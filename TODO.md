...
- [ ] **鑑定・命名・発見リスト忠実度**
  ...
  - [ ] `scrolls.c:read_scroll()` 準拠で、使用時に正式判明する/しない scroll と `call_it()` 相当を分ける（`S_CONFUSE` / `S_SLEEP` / `S_SCARE` / `S_FDET` / `S_TELEP` / `S_CREATE` / `S_REMOVE` / `S_AGGR` / `S_PROTECT` / `S_HOLD` / `S_ARMOR` / `S_ENCH` など一部対応済み）
