...
- [ ] 近接メッセージログの wiki 本文を整備する
  - 進行方向予測、3x3量子化、後方除外、ラッチ、フェード、Log画面救済を説明する
- [x] 通常画面のミニマップを廃止
...
- [x] 80桁表示、四隅HUD、近接toastを 512×320 / Pyxel Web / 携帯機向けに再調整
- [x] カメラ追従開始位置の調整
...
- [x] BGM 導入方式検討
  ...
  - [x] 実行時生成 + `フロア帯候補 × 階番号 × S` キャッシュ方式で導入
...
- [ ] GBC風インタフェース刷新（優先度低。詳細は `archive/DESIGN_ARCHIVE.md` 参照）
  ...
  - [x] 複数パレット定義（標準Flexoki Dark + 2候補）
    - Flexoki Darkを標準にし、GBC High Contrast、Flexoki Lightを比較候補として残す。
  ...
  - [x] 8bit BGM Generator 導入方針確定（実行時生成、フロア帯候補/階番号/状態ごとに同一プレイ中キャッシュ）
