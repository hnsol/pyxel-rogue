...
タイトルBGMは `rogue.py` の `TITLE_BGM_MMLS` で管理する。ダンジョンBGMは `pyxel_rogue/rogue_bgm.py` がフロア深度と HP / 空腹状態から選び、`pyxel_rogue/rogue_bgm_generator.py` で実行時生成する。生成データは `assets/bgm_data/` に置き、ゲーム中は階番号と状態 `S` ごとに一度だけ生成してキャッシュする。同じ低層プロファイル内でも、階が変わったら新しい探索BGMを曲頭から再生する。
