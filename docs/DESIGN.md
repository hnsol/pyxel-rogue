...
スコアボードは難易度別に分ける。金塊生成、所持金、勝利時の pack 売却価値、死亡時の90%換算は Rogue 5.4.4 の計算を維持し、難易度倍率は掛けない。将来調整用に難易度プロファイルは `score_multiplier=1.0` を持つが、現時点では表示スコアも登録スコアも単一値とする。オンラインの dummy score は空欄を避ける表示補助なので、実プレイヤーの難易度分離とは別に全難易度で表示対象にする。
