...
class TestSfxController(unittest.TestCase):
    def test_request_plays_highest_priority_sound_on_update(self):
        from pyxel_rogue.rogue_sfx import SFX_CH, SFX_HEAL_LARGE, SFX_KILL, SfxController

        px = FakePyxel()
        ctrl = SfxController(px)
        ctrl.request(SFX_KILL)
    ...
    def test_update_resumes_bgm_after_active_sound_finishes(self):
        from pyxel_rogue.rogue_sfx import SFX_CH, SFX_KILL, SfxController
    ...
    def test_update_does_not_resume_bgm_while_sound_is_playing(self):
        from pyxel_rogue.rogue_sfx import SFX_KILL, SfxController
    ...
    def test_copy_se_pack_slots_shifts_4_to_33_into_8_to_37(self):
        from pyxel_rogue.rogue_sfx import copy_se_pack_slots

        px = FakePyxel()
        px.sounds[4] = FakeSound("c", "s", "7", "n", 3)
        px.sounds[33] = FakeSound("d", "p", "6", "f", 4)

        copy_se_pack_slots(px)

        self.assertEqual(px.sounds[8].set_calls, [("c", "s", "7", "n", 3)])
        self.assertEqual(px.sounds[37].set_calls, [("d", "p", "6", "f", 4)])
