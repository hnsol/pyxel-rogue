...
class RogueBaselineTest(unittest.TestCase):
    ...
    def test_ui_tables_are_split_without_changing_menu_actions(self):
        ...
        self.assertEqual(rogue_ui.pack_grid_pos(9, 26), (1, 0))
        self.assertEqual(rogue_ui.pack_grid_pos(25, 26), (2, 7))
        self.assertEqual(rogue_ui.pack_grid_move(0, 1, 0, 26), 9)
        self.assertEqual(rogue_ui.pack_grid_move(9, -1, 0, 26), 0)
        self.assertEqual(rogue_ui.pack_grid_move(0, 0, -1, 26), 8)
        self.assertEqual(rogue_ui.pack_grid_move(8, 0, 1, 26), 0)
    ...
    def test_ui_text_width_uses_layout_font_metrics(self):
        game = new_game(seed=263)

        old_ascii, old_cjk = rogue.FONT_ASCII_W, rogue.FONT_CJK_W
        try:
            rogue.FONT_ASCII_W = 7
            rogue.FONT_CJK_W = 12
            self.assertEqual(game.ui_text_width("AB薬"), 26)
        finally:
            rogue.FONT_ASCII_W = old_ascii
    ...
    def test_settings_save_load_round_trips_language_palette_and_pickup(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as td:
                rogue_lang.SETTINGS_FILE = os.path.join(td, "settings.json")
                rogue_lang.save_settings(
                    rogue.Settings(
                ...
                loaded = rogue_lang.load_settings()
    ...
    def test_option_changes_are_saved_and_auto_pickup_survives_new_game(self):
        game = new_game(seed=3471)
        saved = []
        old_save = rogue.save_settings
        try:
            rogue.save_settings = lambda settings: saved.append(
                (settings.language, settings.palette, settings.auto_pickup)
            )

            game.toggle_lang()
            self.assertEqual(saved[-1][0], rogue.LANG_JA)

            game.toggle_palette()
            self.assertEqual(saved[-1][1], game.settings.palette)

            game.toggle_auto_pickup()
            self.assertFalse(saved[-1][2])
            game.new_game()
    ...
    def test_hades_hud_uses_corner_status_and_bonus_equipment(self):
        ...
        game.p.level = 5
        game.p.exp = 135
        game.p.wpn = rogue.Item(rogue.CAT_WPN, 0, hit_plus=1, dam_plus=1)
        calls = []
    ...
    def test_nyandor_online_scores_from_dedicated_endpoint_fill_variant_for_display(self):
        old_variant = rogue.GAME_VARIANT
        old_fetch = rogue.fetch_online_scores
        old_save_cache = rogue.save_online_score_cache
        try:
            ...
            game.settings = rogue.Settings(difficulty=rogue.DIFF_NORMAL)
            game.lang = rogue.LANG_EN
            game.online_profile = {"user_name": "cat", "local_only": False, "server_token": "tok"}
            game.online_score_cache = {}
            game.online_score_loaded = set()
            rogue.fetch_online_scores = lambda *args, **kwargs: [
                {
                    "score": 555,
                    "player_name": "cat",
                    "period_week": rogue.score_period_keys()["period_week"],
                    "result_flags": "winner",
                    "level": 5,
                }
            ]
        ...
        self.assertEqual(scores[0]["variant"], "nyandor")
        self.assertEqual(
            game.format_score_line_for_board(1, scores[0], rogue.SCOREBOARD_PERIOD_WEEKLY),
            " 1   555 cat: returned with the Nyandor cat.",
        )

    def test_rogue_online_scores_do_not_fill_nyandor_variant(self):
        old_variant = rogue.GAME_VARIANT
        old_fetch = rogue.fetch_online_scores
        old_save_cache = rogue.save_online_score_cache
        try:
            rogue.GAME_VARIANT = rogue.VARIANT_ROGUE
            game = rogue.Game.__new__(rogue.Game)
            game.settings = rogue.Settings(difficulty=rogue.DIFF_NORMAL)
            game.lang = rogue.LANG_EN
            game.online_profile = {"user_name": "cat", "local_only": False, "server_token": "tok"}
        ...
        self.assertNotIn("variant", scores[0])
        self.assertEqual(
            game.format_score_line_for_board(1, scores[0], rogue.SCOREBOARD_PERIOD_WEEKLY),
            " 1   555 cat: A total winner on level 5.",
        )
    ...
    def test_flexoki_dark_uses_readable_terrain_and_gold_colors(self):
        ...
        self.assertEqual(rogue.FLEXOKI_DARK_PALETTE[2], 0x4F3685)
        ...
        self.assertEqual(rogue.UI_SUBTEXT_COL, 4)
        self.assertEqual(rogue.palette_monster_color("flexoki_dark", "I"), 13)
        self.assertEqual(rogue.palette_monster_color("flexoki_dark", "P"), 2)
        self.assertEqual(rogue.palette_monster_color("flexoki_dark", "U"), 2)
        self.assertEqual(rogue.palette_monster_color("flexoki_dark", "X"), 2)

    def test_flexoki_light_uses_readable_monster_colors(self):
        game = new_game(seed=242)
        game.settings.palette = "flexoki_light"

        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[0], 0xE6E4D9)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[1], 0xDAD8CE)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[2], 0x5E409D)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[3], 0xB7B5AC)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[4], 0x878580)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[5], 0xB7B5AC)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[6], 0xA0AF54)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[7], 0x879A39)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[8], 0xBC5215)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[12], 0x24837B)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[11], 0xD0A215)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[13], 0x4385BE)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[14], 0x403E3C)
        self.assertEqual(rogue.FLEXOKI_LIGHT_PALETTE[15], 0x282726)
        self.assertEqual(rogue.PALETTE_THEMES["flexoki_light"].roles[rogue.ROLE_MEMORY], 5)
        ...
        self.assertEqual(game.monster_color("P"), 13)
        self.assertEqual(game.monster_color("U"), 13)
        self.assertEqual(game.monster_color("X"), rogue.palette_monster_color("flexoki_light", "X"))

    def test_flexoki_syntax_dark_keeps_dark_structure_with_400_accents(self):
        game = new_game(seed=243)
        game.settings.palette = "flexoki_syntax_dark"

        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[3], 0x343331)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[4], 0x6F6E69)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[5], 0x878580)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[2], 0x4F3685)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[7], 0x879A39)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[8], 0xDA702C)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[9], 0xD14D41)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[10], 0xCE5D97)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[11], 0xD0A215)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[12], 0x3AA99F)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_DARK_PALETTE[13], 0x4385BE)
        self.assertEqual(game.monster_color("I"), 13)
        self.assertEqual(game.monster_color("P"), 2)
        self.assertEqual(game.monster_color("U"), 2)
        self.assertEqual(game.monster_color("X"), 10)

    def test_flexoki_syntax_light_keeps_light_structure_with_400_accents(self):
        ...
        self.assertEqual(rogue.FLEXOKI_SYNTAX_LIGHT_PALETTE[13], 0x4385BE)
        self.assertEqual(rogue.FLEXOKI_SYNTAX_LIGHT_PALETTE[14], 0x403E3C)
        self.assertEqual(rogue.PALETTE_THEMES["flexoki_syntax_light"].roles[rogue.ROLE_MEMORY], 5)
        self.assertEqual(game.monster_color("B"), 6)
        self.assertEqual(game.monster_color("I"), 13)
    ...
    def test_light_palette_explored_memory_uses_darker_base_color(self):
    ...
    def test_item_picker_draws_pack_as_grid_with_current_cursor(self):
        ...
        item_lines = [c for c in calls if len(c[2]) >= 4 and c[2][2:4] == ") "]
        self.assertTrue(any(s.startswith(" a)") for _, _, s, _ in item_lines))
        selected = next(c for c in item_lines if c[2].startswith(">n)"))
        first = next(c for c in item_lines if c[2].startswith(" a)"))
        self.assertGreater(selected[0], first[0])
        self.assertEqual(selected[1], first[1])

    def test_pack_grid_does_not_overtrim_item_names_in_picker_or_inventory(self):
        game = new_game(seed=3711)
        game.p.inv = [rogue.Item(rogue.CAT_FOOD, 0)]
        game.fitems = list(game.p.inv)
        game.cact = "Drop"
        game.item_name = lambda it: "abcdefghijklmnopqrstuvwxyz"
        calls = []
        game.txt = lambda x, y, s, c: calls.append(str(s))
    ...
    def test_item_picker_restores_previous_object_and_marks_cursor_cyan_until_moved(self):
        ...
        game.p.inv = [rogue.Item(rogue.CAT_FOOD, 0) for _ in range(rogue.INV_MAX)]
        target = game.p.inv[9]
        game.last_item_by_action["Drop"] = target

        game.start_item_action("Drop")

        self.assertEqual(game.icur, 9)
        self.assertTrue(game.item_cursor_restored)
        ...
        self.assertEqual(game.icur, 10)
        self.assertFalse(game.item_cursor_restored)

    def test_item_picker_missing_previous_object_starts_at_a_without_gold_cursor(self):
        game = new_game(seed=373)
        old_item = rogue.Item(rogue.CAT_FOOD, 0)
        game.p.inv = [rogue.Item(rogue.CAT_FOOD, 0), rogue.Item(rogue.CAT_FOOD, 0)]
        game.last_item_by_action["Drop"] = old_item

        game.start_item_action("Drop")

        self.assertEqual(game.icur, 0)
        self.assertFalse(game.item_cursor_restored)
    ...
    def test_message_toast_uses_recent_rows_with_turn_age_colors(self):
        game = new_game(seed=35)
        calls = []
        game.txt = lambda x, y, s, c: calls.append((str(s), c))
    ...
    def test_light_palette_message_toast_keeps_four_fade_stages_without_black(self):
        game = new_game(seed=36)
        game.settings.palette = "flexoki_light"
        calls = []
        game.txt = lambda x, y, s, c: calls.append((str(s), c))
        game.msgs = ["faint", "dim", "soft", "fresh", "latest"]
        game.msg_turns = [1, 2, 4, 5, 6]
        game.turn = 6

        game.draw_msgs()

        calls = [(s, c) for s, c in calls if c != rogue.MSG_TOAST_SHADOW_COL and s not in (">", "!")]
        self.assertEqual([color for _, color in calls], [3, 3, 5, 4, 14])
        self.assertNotIn(6, [color for _, color in calls])
        self.assertEqual(
            [
                rogue.msg_toast_color(0, "flexoki_light"),
    ...
    def test_important_message_blocks_play_until_acknowledged(self):
        ...
        self.assertFalse(game.message_ack_pending)
    ...
    def test_level_up_message_is_important(self):
        ...
        game.p.level = 1
        game.p.exp = game.p.EXP_T[1]
        self.assertTrue(game.p.lvlup())
        game.msg_important("misc.welcome_to_level_level", level=game.p.level)
    ...
    def test_message_toast_does_not_use_dithered_panel(self):
        ...
        rect_calls = []
        rectb_calls = []
        old_dither, old_rect, old_rectb = rogue.pyxel.dither, rogue.pyxel.rect, rogue.pyxel.rectb
        try:
            rogue.pyxel.dither = lambda alpha=1.0: dither_calls.append(alpha)
            rogue.pyxel.rect = lambda *args: rect_calls.append(args)
            rogue.pyxel.rectb = lambda *args: rectb_calls.append(args)
            game.txt = lambda *args: None
            game.draw_msgs()
        finally:
            rogue.pyxel.dither = old_dither
            rogue.pyxel.rect = old_rect
            rogue.pyxel.rectb = old_rectb
    ...
    def test_online_user_and_score_sync_can_post_variant_tab_key(self):
        from pyxel_rogue import rogue_scores

        calls = []
        old_http_json = rogue_scores._http_json
        try:
            rogue_scores._http_json = lambda url, payload=None: calls.append(payload) or {"ok": True, "exists": False}
            rogue_scores.check_online_user("cat", "https://example.test/exec", variant="nyandor")
            rogue_scores.register_online_user("cat", "123456", "https://example.test/exec", variant="nyandor")
            rogue_scores.link_online_user("cat", "123456", "https://example.test/exec", variant="nyandor")
            rogue_scores.sync_online_scoreboard(
                {"user_name": "cat", "server_token": "tok"},
                [{"score": 30, "player_name": "cat", "period_week": "2026-W18"}],
                "https://example.test/exec",
                variant="nyandor",
            )
            rogue_scores.record_guest_scoreboard_sync("https://example.test/exec", variant="nyandor")
        finally:
            rogue_scores._http_json = old_http_json

        self.assertEqual([payload["variant"] for payload in calls], ["nyandor"] * 5)
    ...
    def test_apps_script_scoreboard_deduplicates_score_id_before_append(self):
        ...
        self.assertIn("scoreIdExists(scoreId, variant)", script)

    def test_apps_script_keeps_score_sheet_schema_without_variant_column(self):
        path = os.path.join(ROOT, "docs", "apps_script_scoreboard.gs")
        with open(path, encoding="utf-8") as f:
            script = f.read()

        sheet_block = script[script.index("function sheet(variant)"):script.index("function scoreContext(variant)")]
    ...
    def test_apps_script_get_uses_single_score_context_for_dummy_and_top_scores(self):
        ...
        self.assertIn("function scoreContext(variant)", script)
    ...
    def test_apps_script_sync_scoreboard_does_not_generate_dummy_or_payload_scores(self):
        ...
        self.assertIn("const firstScorePost = entries.length > 0 && !hasUserScore(userName, body.variant)", sync)
        self.assertIn("if (next && new Date(next).getTime() > Date.now() && !firstScorePost)", sync)
        self.assertIn("function hasUserScore(userName, variant)", script)

    def test_apps_script_records_guest_scoreboard_sync_metrics_separately(self):
        path = os.path.join(ROOT, "docs", "apps_script_scoreboard.gs")
        with open(path, encoding="utf-8") as f:
            script = f.read()

        self.assertIn('const GUEST_METRICS_SHEET_NAME = "guest_metrics"', script)
        self.assertIn('if (body.action === "guestScoreboardSync") return json(recordGuestScoreboardSync(body.variant))', script)
        self.assertIn("function guestMetricsSheet(variant)", script)
        self.assertIn("function recordGuestScoreboardSync(variant)", script)
        self.assertIn('"timestamp"', script)
        self.assertIn('"event"', script)
        self.assertIn('"scoreboard_sync"', script)
    ...
    def test_pages_deploy_uses_single_score_url_for_variant_tabs(self):
        path = os.path.join(ROOT, "tools", "deploy_pages.sh")
        with open(path, encoding="utf-8") as f:
            script = f.read()

        self.assertIn("PYXEL_ROGUE_SCORE_URL", script)
        self.assertNotIn("PYXEL_ROGUE_SCORE_URL_NYANDOR", script)
        self.assertNotIn("PYXEL_ROGUE_SCORE_URL_ROGUE", script)
        self.assertNotIn('env -u PYXEL_ROGUE_SCORE_URL', script)
        self.assertIn("Building Rogue Web deploy with PYXEL_ROGUE_SCORE_URL", script)
    ...
    def test_online_register_rejects_blank_name_without_server_check(self):
        ...
        try:
            rogue.check_online_user = lambda user_name, **kwargs: self.fail("blank name must not hit server")
    ...
    def test_online_register_rejects_guest_name_without_server_check(self):
        ...
        old_check = rogue.check_online_user
        try:
            rogue.check_online_user = lambda user_name, **kwargs: self.fail("guest name must not hit server")
    ...
    def test_online_sync_posts_only_scores_recorded_after_signup_name(self):
        ...
        try:
            ...
            rogue.sync_online_scoreboard = lambda profile, entries, **kwargs: posted.extend(entries) or {"ok": True}
    ...
    def test_online_registered_name_screen_hides_local_only_hint_until_edit(self):
        ...
        game.name_chars = list("newname")
        game.draw_online_register_screen()
        self.assertIn("B/Esc keeps guest mode.", "\n".join(calls))
    ...
    def test_online_register_checks_server_before_pin_for_new_user(self):
        ...
        old_check = rogue.check_online_user
        try:
            rogue.check_online_user = lambda user_name, **kwargs: checked.append(user_name) or {"ok": True, "exists": False}
    ...
    def test_online_register_existing_user_requests_pin_link(self):
        ...
        old_check = rogue.check_online_user
        try:
            rogue.check_online_user = lambda user_name, **kwargs: {"ok": True, "exists": True}
    ...
    def test_online_register_rejects_reserved_names_without_server_check(self):
        ...
        old_check = rogue.check_online_user
        try:
            rogue.check_online_user = lambda user_name, **kwargs: self.fail("reserved names must not hit server")
    ...
    def test_online_pin_link_failure_rejects_name(self):
        ...
        try:
            rogue.link_online_user = lambda user_name, pin, **kwargs: {"ok": False, "status": "auth_failed"}
    ...
    def test_online_score_select_with_cooldown_loads_scoreboard_without_post(self):
        ...
        try:
            ...
            rogue.sync_online_scoreboard = lambda profile, entries, **kwargs: self.fail("cooldown select must not POST sync")
    ...
    def test_guest_scoreboard_sync_records_one_guest_metric_without_blocking_refresh(self):
        game = rogue.Game.__new__(rogue.Game)
        game.settings = rogue.Settings()
        game.online_profile = {"user_name": "guest", "local_only": True, "profile_exists": True}
        game.online_sync_periods = [rogue.SCOREBOARD_PERIOD_WEEKLY, rogue.SCOREBOARD_PERIOD_SEASON]
        recorded = []
        loaded = []
        old_record = rogue.record_guest_scoreboard_sync
        try:
            rogue.record_guest_scoreboard_sync = lambda **kwargs: recorded.append(kwargs.get("variant"))
            game.load_online_period_scores = lambda period, force=False: loaded.append((period, force)) or []

            result = game.perform_online_scoreboard_sync()
        finally:
            rogue.record_guest_scoreboard_sync = old_record
    ...
    def test_guest_scoreboard_sync_ignores_guest_metric_failure(self):
        ...
        old_record = rogue.record_guest_scoreboard_sync
        try:
            rogue.record_guest_scoreboard_sync = lambda **kwargs: (_ for _ in ()).throw(RuntimeError("metric down"))
            game.load_online_period_scores = lambda period, force=False: []
    ...
    def test_online_score_sync_posts_local_scores_inside_scoreboard(self):
        ...
        try:
            ...
            rogue.sync_online_scoreboard = lambda profile, entries, **kwargs: synced.append((profile, entries, kwargs)) or {
                "ok": True,
                "status": "success",
                "last_sync_at": "2026-05-01T00:00:00Z",
                "next_sync_at": "2026-05-02T00:00:00Z",
    ...
    def test_online_score_sync_without_local_scores_skips_post_and_loads_scoreboard(self):
        ...
        try:
            rogue.load_score_entries = lambda: []
            rogue.sync_online_scoreboard = lambda profile, entries, **kwargs: self.fail("empty local score list must not POST sync")
            game.load_online_period_scores = lambda period, force=False: loaded.append((period, force)) or []
    ...
    def test_online_scoreboard_load_failure_does_not_overwrite_sync_success(self):
        ...
        try:
            rogue.load_score_entries = lambda: [{"score": 42, "player_name": "ace", "period_week": "2026-W18"}]
            rogue.sync_online_scoreboard = lambda profile, entries, **kwargs: {"ok": True, "status": "success"}
    ...
    def test_online_score_server_cooldown_loads_scoreboard_without_sync_success(self):
        ...
        try:
            rogue.load_score_entries = lambda: [{"score": 42, "player_name": "ace", "period_week": "2026-W18"}]
            rogue.sync_online_scoreboard = lambda profile, entries, **kwargs: {"ok": False, "status": "cooldown"}
            game.load_online_period_scores = lambda period, force=False: loaded.append((period, force)) or []
    ...
    def test_online_score_tabs_do_not_sync_and_refresh_syncs_all_periods(self):
        ...
        try:
            rogue.load_score_entries = lambda: []
            rogue.sync_online_scoreboard = lambda profile, entries, **kwargs: calls.append((profile, entries, kwargs)) or {
                "ok": True,
                "status": "success",
                "scores": {"weekly": [], "season": []},
            }
    ...
    def test_guest_online_score_fetch_keeps_remote_scores_separate_from_local_guest_scores(self):
        game = rogue.Game.__new__(rogue.Game)
        game.settings = rogue.Settings()
        game.online_profile = {"user_name": "guest", "local_only": True, "profile_exists": True}
        game.online_period = rogue.SCOREBOARD_PERIOD_WEEKLY
        ...
        try:
            ...
            rogue.load_score_entries = lambda: [
                {"player_name": "guest", "score": 300, "period_week": keys["period_week"], "period_season": keys["period_season"]}
            ]
        ...
        self.assertEqual([(e["player_name"], e["score"]) for e in scores], [("dot", 20)])
        self.assertEqual([(e["player_name"], e["score"]) for e in game.display_online_period_scores(rogue.SCOREBOARD_PERIOD_WEEKLY)], [("dot", 20)])
    ...
    def test_online_score_draw_keeps_guest_out_and_ranks_current_user_local_best(self):
        ...
        key = rogue.score_period_keys()["period_season"]
        game.online_score_cache = {
            rogue.SCOREBOARD_PERIOD_SEASON: [
                {"score": 2159, "player_name": "fkfksan", "period_season": key, "result_flags": "winner", "level": 5},
                {"score": 599, "player_name": "palette", "period_season": key, "result_flags": "killed", "level": 5, "killer": "zombie"},
            ]
        }
    ...
    def test_diag_assist_does_not_block_menu_vertical_navigation(self):
        ...
        game.st = rogue.ST_ITEM
        game.fitems = [rogue.Item(rogue.CAT_FOOD, 0) for _ in range(rogue.INV_MAX)]
        game.icur = 0
        rogue.pyxel.set_input(
            held={rogue.pyxel.GAMEPAD1_BUTTON_DPAD_RIGHT},
            pressed={rogue.pyxel.GAMEPAD1_BUTTON_DPAD_RIGHT},
    ...
    def test_action_menu_draws_keyboard_hints_in_english_and_japanese(self):
        ...
        self.assertTrue(any("z)Zap" in s for s in calls))
        self.assertTrue(any("t)Throw" in s for s in calls))

        game.lang = rogue.LANG_JA
        calls.clear()
        game.draw_menu()

        self.assertTrue(any("z)振る" in s for s in calls))
        self.assertTrue(any("t)投げる" in s for s in calls))

    def test_command_windows_have_staged_positions(self):
        game = new_game(seed=4722)
        boxes = []
        game._box = lambda x, y, w, h, title="": boxes.append((x, y, w, h, title))
        game.txt = lambda *args: None
        ...
        map_center_x = rogue.ZV_X + rogue.ZV_PX_W // 2
        map_center_y = rogue.ZV_Y + rogue.ZV_PX_H // 2
        action_center = (action_box[0] + action_box[2] // 2, action_box[1] + action_box[3] // 2)
        ...
        self.assertGreater(dir_box[0], action_box[0])

    def test_command_chain_draws_previous_windows_under_current_prompt(self):
        game = new_game(seed=4723)
        game.action_origin = rogue.ST_MENU
        game.cact = "Throw"
        game.dact = None
        game.throw_dir = (1, 0)
        game.st = rogue.ST_ITEM
        game.p.inv = [rogue.Item(rogue.CAT_FOOD, 0) for _ in range(2)]
        game.fitems = list(game.p.inv)
        boxes = []
    ...
    def test_command_chain_cancel_removes_only_top_window(self):
        ...
        game.cancel_item_prompt()

        self.assertEqual(game.st, rogue.ST_DIR)
        self.assertEqual(game.dact, "Throw")

        game.cancel_dir_prompt()

        self.assertEqual(game.st, rogue.ST_MENU)

    def test_quick_throw_item_cancel_returns_to_direction_prompt(self):
        game = new_game(seed=4725)
        game.action_origin = rogue.ST_PLAY
        game.cact = "Throw"
        game.dact = None
        game.throw_dir = (1, 0)
        game.st = rogue.ST_ITEM
    ...
    def test_rogue_544_again_command_repeats_search(self):
        ...
        self.assertEqual(game.turn, 2)
        self.assertEqual(game.msgs, msgs_before)
    ...
    def test_rogue_544_count_prefix_is_cleared_by_non_countable_command(self):
        ...
        self.assertEqual(game.turn, 1)
        self.assertTrue(game.msgs[-1].startswith("version "))
    ...
    def test_player_hp_is_clamped_to_zero_after_lethal_trap_damage(self):
        game = new_game(seed=641)
        set_open_floor(game)
        x, y = game.p.x + 1, game.p.y
        kind = next(i for i, t in enumerate(rogue.TRAPS) if t["name"] == "arrow trap")
