...
メッセージ履歴画面は `Select -> Select -> Log` から開く。直近50〜100件程度を上下スクロールで確認し、Bで戻る。ターンは消費しない。フィルタ、検索、敵一覧、危険警告などは持たせず、常時下ログを補完する小さな保険に留める。
...
## 将来候補：GBC 美学寄せと 360x240 移行

この章は現在の優先方針ではない。GBC風UI、360×240、狭い画面、小型ゲーム機最適化、全体マップオーバーレイは `DESIGN_ARCHIVE.md` に退避する将来候補として扱う。当面は PC / SteamDeck 想定の現行 UI を磨き込む。
