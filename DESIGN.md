...
Pyxel Web Launcher で GitHub branch を指定して起動確認する場合、branch 名には `/` を使わない。Launcher の `run=owner/repo/branch/path` 形式では、`codex/foo` のような slash 付き branch 名が path と誤解されることがあるため、Web確認用には `web-...` のような slash なし branch を push する。
