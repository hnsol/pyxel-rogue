...
元祖ローグの3×3セクターグリッドアルゴリズムを再現。現行では48×24マップを16×8セクターに分割し、9セクターに `Room` を作ったうえで Rogue 5.4.4 `rooms.c` 相当の `ISGONE` / 暗い部屋 / `ISMAZE` フラグを割り当てる。通常部屋、迷路部屋、gone room を隣接セクター間で通路接続し、flood fill で到達保証している。
