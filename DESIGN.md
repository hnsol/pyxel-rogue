...
現行 Pyxel 版は `IdentTable.pk/sk/rk/wk` と `Item.known` に加え、`oi_guess` 相当の `pg/sg/rg/wg`、`misc.c:call_it()` 相当の `c` call、`things.c:print_disc()` 相当の `D` discovered list まで実装済み。weapon / armor の個体ラベル `o_label` 接続は未完了。`use_pot()` / `use_scr()` は原作より早く正式名を判明させるものがある。ring / stick は Rogue 5.4.4 `things.c:nameit()`、`rings.c:ring_num()`、`sticks.c:charge_str()` に合わせ、種類判明後も個体 `ISKNOW` 相当の `Item.known` が立つまで補正値・チャージを隠す。
...
命名機能と思い出し機能は、potion / scroll / ring / stick の `oi_guess` と discovered list まで実装済み。残りは `command.c:call()` の weapon / armor 個体単位 `o_label` 接続と、Inventory 表示・identify scroll・日英文言への波及確認である。`things.c:print_disc()` は `oi_know` または `oi_guess` がある potion / scroll / ring / stick を一覧化する。
