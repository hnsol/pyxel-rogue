...
Action menu は、繰り返しやすく誤爆リスクが低い `Quaff` / `Read` / `Eat` / `Zap` / `Throw` だけ前回Actionを復元する。復元カーソルは `UI_RESTORED_CURSOR_COL` で表示し、ユーザーが上下左右へ動かした時点で通常の選択色へ戻す。装備変更、drop、call、discoveries などは初期選択 `Eat` に戻す。
