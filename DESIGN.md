...
現行の 420×280 画面、48×24 マップ、3×3セクターグリッドは実装値であり、固定仕様ではない。移植性のため、レイアウト定数は将来的に画面サイズから再計算できるようにする。

## 画面レイアウト

現行レイアウトは、左側に情報列、右側にメインのASCIIダンジョン表示を置く。左情報列は上からミニマップ、ステータス、メッセージの順にまとめる。メイン表示は 34×20 タイルを維持し、ASCII ローグライクとしての可読性を優先する。

ミニマップは 3px タイルで表示し、探索済み/視界内の地形に加えて階段を見分けられるようにする。メッセージ欄は左情報列幅に合わせて折り返し、最大6行ぶん表示する。詳細な履歴表示は後続のメッセージ履歴スクロールで扱う。
...
Select(Back) はフルマップ直行ではなく補助メニューにする。マップ、ステータス、ヘルプ、search の入口を1か所にまとめることで、A/B/Start/Select + D-pad だけで主要操作を完結させる。
...
1歩ごとの中央追従は酔いやすいため、現行はデッドゾーン方式。現在の実装値は横 `DEAD_ZONE_X = 8`、縦 `DEAD_ZONE_Y = 5`。横は 34 タイル表示に対しておおむね 25% / 50% / 25% の比率で、中央付近ではスクロールせず、端へ寄ったら追従する。
