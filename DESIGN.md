...
magic missile は `sticks.c:WS_MISSILE` の `o_hurldmg="1x4"`, `o_hplus=100`, `o_dplus=1`, `ISMISL`, `o_launch=cur_weapon->o_which` と `fight.c:roll_em()` を基準にし、現在武器がある場合はその `o_dplus` も加える。
