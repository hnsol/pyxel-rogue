...
Scroll の正式鑑定も Rogue 5.4.4 `scrolls.c:read_scroll()` の各分岐に合わせる。`S_HOLD` は周囲2マス以内の running monster を実際に `ISHELD` 相当へしたときだけ `scr_info[S_HOLD].oi_know` を立て、対象がいない場合は strange sense of loss で未鑑定のままにする。`S_TELEP` は `teleport()` 後に `cur_room != proom` になった場合だけ識別する。`S_ARMOR` / `S_ENCH` は装備中の防具 / 武器がある場合だけ `scr_info[] .oi_know` を立て、対象装備がない場合は同じく strange sense of loss にする。
