...
スコア保存はゲームメカニクスの外側に置く。`vendor/rogue544/rip.c:score()` と `main.c:quit()` を基準に、順位は Gold のみ、死亡は 90%、quit と勝利は 100% を採用する。ローカル保存は JSON / localStorage とし、オンライン v1 は Google Sheets + Apps Script へ任意投稿する。スコアボードは Daily Top Ten / Weekly Rivals / Season Legends を切り替え、Daily は UTC 日付、Weekly / Season は同一期間・同一 player_name の最高点だけを表示する。Season は Spring 3-5月、Summer 6-8月、Fall 9-11月、Winter 12-2月の固定季節制とする。Apps Script の雛形は `docs/apps_script_scoreboard.gs` に置き、公開先URLは `PYXEL_ROGUE_SCORE_URL` で差し替えられるようにする。スコアボードへ入るたびに Daily / Weekly / Season をまとめて同期してキャッシュし、タブ切替では通信せず、R / Select で3期間を手動再同期する。オンライン投稿はゲーム終了時だけ行い、各スコアに `score_id` を付け、Apps Script 側でも同一 `score_id` の重複行を追加しない。Pyxel Rogue は起動ロゴ中に `?action=seedDummy` を呼ぶだけにし、Apps Script が表示Top10に足りない分だけダミー行を生成して Sheet へ一括書き込みする。投稿失敗時はゲーム進行を止めずローカル表示へ戻す。
