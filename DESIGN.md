...
食料は Rogue 5.4.4 `misc.c:eat()` を基準にする。食べた時に `food_left` を `max(food_left, 0) + HUNGERTIME - 200 + rnd(400)` で増やし、`STOMACHSIZE` で丸め、`hungry_state=0` 相当へ戻す。slime-mold は専用の yummy fruit 文言、ration は `rnd(100)>70` で `s_exp++` と awful 文言、それ以外は good 文言にする。
