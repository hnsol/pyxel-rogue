...
メッセージは下ログではなく、`message toast` としてダンジョン上に最大5行だけ一時表示する。新規メッセージ発生時に濃く表示し、一定ターン数で暗くなり、その後消える。フレーム数ではなくターン数基準にし、操作環境や描画フレームレートの差で消える速さが変わりすぎないようにする。表示位置は左下を基本にし、プレイヤー周辺の横±10タイル・縦±5タイルを隠しそうなら右下、それでも隠しそうなら左上へ逃がす。戻る時は少し広めの解除範囲でヒステリシスをかけ、境界付近のちらつきを抑える。流れたログはメッセージ履歴画面で確認できるようにし、toast 自体に検索やフィルタは持たせない。
