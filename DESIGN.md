...
toast の位置は `80×22` ダンジョンを3×3ブロックに分割し、プレイヤー `@` のいるブロックの上下左右いずれかへ出す。斜めブロックは使わない。新着メッセージ発生時、またはプレイヤーがtoastブロック外側1マスの余白へ入ったときだけ位置を再評価する。直前の有効移動方向があれば、その逆側ブロックを優先する。無効な場合は下、上、左、右の順で選ぶ。戦闘や足踏みなど移動しないターンでは、基本的に位置を変えない。

toast は枠と半透明フィルを持たず、テキストと1px shadowだけで描く。幅は23 ASCII文字相当を基準にし、英語は単語途中で切らないようスペース直前で折り返す。CJKを含む文はスペースではなく pixel幅で折り返し、行頭 `、。！？` を避ける。日本語文末の `。` はtoast表示時に省き、意味を持つ `！` / `？` は残す。新しいメッセージは下へ追加し、古いメッセージが上、新しいメッセージが下に来る時間順ログとして読む。0ターンの各メッセージ先頭には `>` を置き、wrap継続行にはマーカーを付けない。ack必須メッセージは未承認中だけ行頭 `!` をブリンクさせ、本文色も強調色にする。承認後は通常の `>` 表示へ戻す。流れたログはメッセージ履歴画面で確認できるようにし、toast 自体に検索やフィルタは持たせない。
