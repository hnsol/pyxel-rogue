...
命名機能と思い出し機能は、potion / scroll / ring / stick の `oi_guess`、weapon / armor 個体単位 `o_label`、discovered list まで実装済み。`things.c:print_disc()` は `oi_know` または `oi_guess` がある potion / scroll / ring / stick を一覧化する。`potions.c:quaff()` と `scrolls.c:read_scroll()` の末尾は `misc.c:call_it()` を呼ぶため、使用後に `oi_know` が立った potion / scroll は既存の `oi_guess` を消し、未判明かつ仮名ありなら維持する。
