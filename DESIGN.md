...
斜め移動・斜め近接攻撃・モンスターの斜め接触攻撃は Rogue 5.4.4 の `diag_ok()` 相当で判定する。斜め先そのものに加え、直交する2マスがどちらも通行可能なときだけ成立する。これにより、扉上や壁角から直交片側が壁になっている斜め先へは移動できず、そこにいるモンスターへの近接攻撃も発生しない。`move.c:do_move()` に合わせ、不正斜め移動はターン非消費で run だけ停止する。未発見 floor trap は `winat()==FLOOR` の時だけ Venus Flytrap held gate より先に発動し、monster が乗っている hidden trap では held gate が勝つ。held 中でも目的地表示が `F` なら `ch != 'F'` の gate を通過するため、保持者以外の Venus Flytrap へも攻撃できる。`DOOR`、`STAIRS`、床上 item の `winat()` default 分岐では移動成立時に run を停止する。モンスター側も同じ制約を受ける。

`f/F` の fight-to-death は Rogue 5.4.4 `command.c:command()` の `get_dir()`、隣接 `moat()`、`see_monst()` / `SEEMONST`、`ISTARGET`、`to_death` / `kamikaze`、`runch=dir_ch` の流れに合わせる。方向キャンセルと対象なしは `after=FALSE` とし、対象ありでは同じ方向の近接攻撃を入力前に自動継続する。`fight.c:attack()` に合わせ、非 kamikaze は被弾最大値 `max_hit` を記録し現在HPがそれ以下になったら `to_death` を止め、`fight.c:remove_mon()` に合わせ target monster の死亡・除去で `to_death` / `kamikaze` を解除する。Pyxel 版の表示・入力上は `f` と `F` をキーボードコマンドとして公開し、gamepad には既存のバンプ攻撃と補助メニュー中心の操作を維持する。

`a` の again command は Rogue 5.4.4 `command.c:command()` の `last_comm`、`misc.c:get_dir()` の `last_dir`、`pack.c:get_item()` の `last_pick` / `reset_last()` の流れに合わせる。Pyxel 版では keyboard `a` だけを入口とし、補助メニューには置かない。最後のプレイ中 command、方向指定 command の方向、item command の選択 item を記録し、`v` / `i` / `?` / `D` などの `after=FALSE` command と illegal command も `last_comm` として扱う。方向 prompt を ESC / B で中断した場合は直前 command を復元する。item command repeat では、pack が空なら `you aren't carrying anything`、pack は残っているが `last_pick` がないなら `you ran out` とし、どちらも原作通り `after=FALSE` にせずターンを進める。Select 系や補助メニュー選択は携帯機向け UI として扱い、again の command 入力そのものにはしない。
