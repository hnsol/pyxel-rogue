...
メッセージは下ログではなく、`message toast` としてダンジョン上に最大5行だけ一時表示する。新規メッセージ発生時に濃く表示し、一定ターン数で暗くなり、その後消える。フレーム数ではなくターン数基準にし、操作環境や描画フレームレートの差で消える速さが変わりすぎないようにする。表示位置候補は左下、右下、左上の順にし、各候補へペナルティを付けて最小の位置を選ぶ。同点なら候補順を優先する。ペナルティは @ 本体を隠す場合を最大、@ 周辺横±6・縦±3、visible monster、階段 `%`、発見済み罠、visible item の順に重くする。monster detection だけで見えている敵は低めに扱い、左上そのものにも高めのペナルティを置いて、できるだけ下に居続ける。現在位置から変わる場合にも小さなペナルティを置き、境界付近のちらつきを抑える。流れたログはメッセージ履歴画面で確認できるようにし、toast 自体に検索やフィルタは持たせない。
