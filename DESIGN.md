...
スコア保存はゲームメカニクスの外側に置く。`vendor/rogue544/rip.c:score()`、`rip.c:death()`、`rip.c:killname()`、`rip.c:total_winner()`、`main.c:quit()` を基準に、死亡は Gold の 90% を墓石表示とスコアに使い、starvation / hypothermia / arrow / dart / bolt の死因名と冠詞も `killname()` に合わせる。quit は Gold 100%、勝利は Gold に所持品売却額を加えた値を採用する。ローカル保存は JSON / localStorage とし、オンライン v1 は Google Sheets + Apps Script へ任意投稿する。スコアボードは Daily Top Ten / Weekly Rivals / Season Legends を切り替え、Daily は UTC 日付、Weekly / Season は同一期間・同一 player_name の最高点だけを表示する。Season は Spring 3-5月、Summer 6-8月、Fall 9-11月、Winter 12-2月の固定季節制とする。Apps Script の雛形は `docs/apps_script_scoreboard.gs` に置き、公開先URLは `PYXEL_ROGUE_SCORE_URL` で差し替えられるようにする。ゲーム終了時はローカル保存だけを行い、結果画面表示前に通信しない。スコアボードへ入るたびに Daily / Weekly / Season をまとめて同期してキャッシュし、タブ切替では通信せず、R / Select で3期間を手動再同期する。Online Ranking 入場直後の同期開始待ち中は B でキャンセルできる。オンライン投稿はランキング同期時だけ行い、各スコアに `score_id` を付け、Apps Script 側でも同一 `score_id` の重複行を追加しない。起動ロゴ中は通信せず、Apps Script はスコアボード同期時に必要なperiod/keyだけ、今日10件、過去10日と過去10週を各1件ずつ、安定したダミー行として生成する。Season はそれらの期間行を集計する。投稿失敗時はゲーム進行を止めずローカル表示へ戻す。

タイトル導線は移植UIとして `Logo -> Title -> Play` にする。起動ロゴは仮置きで HSL 表記と U.C.BERKELEY / CSRG 表記を表示し、任意キーでスキップできる。Title の Start は即 New Game に進み、Name 欄選択時だけ英数字8文字の名前入力に入る。ゲーム開始ログは `Hello <NAME>, welcome to the Dungeons of Doom!` / `やあ <NAME>、運命の洞窟へようこそ。` とし、開始直後からマップ/HUD/ログを描画する。
...
ring の未識別 stone は `init.c:init_stones()` を基準にし、`rnd(NSTONES)` の重複回避で `r_stones[]` を選び、対応する `stones[].st_value` を `ring_info[].oi_worth` へ加算する。Pyxel 版では `IdentTable.rstones` と `IdentTable.rworth` を同時に初期化し、`rip.c:total_winner()` 相当の勝利 pack worth では加算後の `rworth` と ring type の正式識別状態を使う。
