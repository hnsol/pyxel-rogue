...
パレットは **32 色**（GBC の BG レイヤー最大 = 8パレット×4色 に対応）。基調は青灰（壁・廊下）と緑（部屋床）の低彩度。警告・戦闘時のみ赤（22番）、UI強調にシアン（27番）、金貨・Gold表示に黄色寄りの琥珀（29番）を使い、常時高彩度にしない。階段は金貨より扉に近い探索目標として扱い、扉と同じ控えめなアンバー（18番）へ寄せる。部屋の縦壁と横壁は同じ可視壁色（4番）で描き、Rogue の記号差だけで壁向きを読ませる。純白（31番）はカーソル・点滅限定。`pyxel.colors` へ実行時に適用する（`Game.__init__` 内）。

Flexoki Dark は標準パレットとして、Flexoki の紙とインクに近い温度感を黒背景の ASCII Rogue に移したものとして扱う。常時大量に出る床、壁、通路、過去ログは 400 番台の明るさを避け、画面全体が発光しない程度に沈める。床は `green-600` 相当の 12番（`#66800B`）で「部屋」を読ませるが、モンスターや Gold より前には出さない。探索済みの記憶地形は 2番、過去ログや弱情報は 6番（`#6F6E69`）へ落とし、現在ログの 30番（`#CECDC3`）と明確に差をつける。

扉と階段は、警告でも報酬でもなく「進行可能性」を示す色として同じ 18番（`#71320D`）を使う。orange-700 相当も比較候補にしたが、常時マップ上に出ると少し主張が強くなるため、標準はより沈んだ orange-800 相当を採用する。Gold、Amulet、重要操作は 29番（`#D0A215`）に集約し、報酬や重要表示として扉・階段より明るく見せる。危険・警告は 22番（`#D14D41`）、魔法や特殊表示は 26番（`#3AA99F`）/ 27番（`#4385BE`）を使い、色相ごとの意味を崩さない。

## モンスター色の方針

黒背景で潰れる `1/2/17/28` はモンスター文字に使わない。通常敵は低層で単調にならないよう、小動物/飛行、人型、野生動物で色を分ける。毒/状態異常は赤橙、魔性/盗み/霊/呪いは深い青緑、最危険枠は琥珀/黄色を使う。

モンスター色は「個体識別」よりも「危険の質」を優先する。A-Z すべてを別色にするのではなく、似た役割の敵は近い色にまとめる。低階層で頻繁に見る敵は青灰、緑、琥珀を散らして単調さを避ける。毒、混乱、能力値低下など、見落とすと判断が変わる特殊攻撃持ちは赤橙や深い青緑へ寄せる。Xeroc や Invisible Stalker のように「見え方」そのものが攻略情報になる敵は、周囲の地形・アイテムと混同しにくいシアン系を使う。

モンスター文字はマップ上で一瞬で読めることを優先するため、背景に沈む暗色、床とほぼ同じ緑、壁とほぼ同じ青灰は避ける。ただし常時強い黄色や白へ寄せると全敵が警告表示に見えるため、最危険枠や現在注目すべき情報に限定する。幻覚中や不可視検知など表示条件が特殊な場合も、追加UIで説明するより、既存の色カテゴリから外れすぎない範囲で差を出す。

## UI配色の方針

UI色は「通常情報は低彩度、操作対象と危険だけ高彩度」を基本にする。HUD、メッセージ、ウィンドウ、設定画面、スコアボードで同じ意味の色を使い回し、画面ごとに別の文法を増やさない。

HUD はプレイ中ずっと視界に入るため、主情報も強すぎる色にしない。HP、Strength、Armor、Gold、Move/Pick などは読める青灰・緑・琥珀で整理し、赤橙は低HP、状態異常、OFF/Manual など「普段と違う」状態に限定する。セクション見出しや現在のモード強調にはシアンを使うが、HUD全体をシアンで埋めない。装備名や効果名は、アイテム本文より状態確認として読ませるので、短く、同じ列で、色より位置で探せるようにする。

ウィンドウとメニューは、枠線を暗い青灰、本文を明るい青灰、選択中・決定・重要操作を琥珀またはシアンで示す。選択肢が並ぶ画面では、未選択項目を強くしすぎず、カーソル位置だけがすぐ分かるようにする。確認ダイアログや設定画面では、危険操作やキャンセル不可状態だけ赤橙を使い、通常の切替項目は赤にしない。

設定画面や補助メニューの色は、ゲームメカニクスではなく表示・入力補助であることが伝わるよう、警告色を控えめにする。Language、Palette、Pickup、Move 補助などはプレイヤー状態を変えるものではないため、戦闘警告と同じ強い赤で扱わない。OFF や Manual は注意喚起として琥珀/赤橙を使ってよいが、異常状態やダメージ表示より弱く見えるようにする。

スコアボードは一覧性を優先する。通常行は明るい青灰、過去行や補助説明は中間青灰、自分の行・今回の結果・通信結果など現在見るべき行だけ琥珀で強調する。通信中や登録確認のウィンドウも、ゲーム本編の危険色とは分け、操作誘導はシアン/琥珀、失敗や制限だけ赤橙にする。

純白は最後の強調手段として扱う。通常本文、HUD、枠線、常時表示には使わず、カーソル、点滅、最重要の一時表示に限定する。黒背景に白を多用すると ASCII マップの読解負荷が上がるため、可読性が必要な箇所でもまず明るい青灰を使う。
...
標準パレットは `flexoki_dark` とする。ゲーム中の Palette 切替は `flexoki_dark`、`gbc_high_contrast`、`flexoki_light` の順で循環する。Flexoki 系は Steph Ango の Flexoki 公式パレットを参照し、紙とインクに近い温度感を Pyxel Rogue の標準表示として採用する。切替は `pyxel.colors` へ即時反映する表示設定であり、ターン消費、RNG、プレイヤー状態、マップ、所持品、モンスター配置には影響させない。
...
Start押下中だけ斜め補助モードにする。短押しトグルより状態の戻し忘れがなく、Start自体を長押し時間で判定しないため携帯機側の電源OFF等とも競合しにくい。斜め補助モードは、SteamDeck、ブラウザ、携帯機で斜め入力を安定させるための移植UIとして扱う。通常時の8方向移動とは別扱いにし、斜め補助モード中は上下左右単体入力では移動しない。ただしこれはプレイ中の移動・ダッシュ・方向指定に限る。メニュー、アイテム選択、補助メニューのカーソル上下は UI 操作として扱い、斜め補助モード中でも上下単体入力を受け付ける。

B はダッシュ（ホールド）とキャンセル/メニュー（単押し）を兼務する。ダッシュ開始は `btnp()` ではなく現在ホールド中の方向で判定し、D-pad を先に押してから B、または B を先に押してから D-pad のどちらでも走り始められるようにする。`btn()` と `btnp()` の扱い、離した瞬間の誤発火には注意する。Start押下中の斜め補助はステータス欄に表示し、現在の入力モードが分かるようにする。

Select(Back) 単押しは頻度の高い持ちもの確認を直接開き、持ちもの表示中にもう一度押すと補助メニューへ進む。Select はゲームパッド向け chord レイヤーとしても扱う。Select+A は Rogue 5.4.4 の `t` コマンドに寄せ、方向を選んでから投げるアイテムを選択する quick throw とする。Select+B は周囲8マス search を実行し、A空押しの正面1マス search と分ける。Select+D-pad は Rogue 5.4.4 の `^` + 方向に相当する Trap Inspect とし、発見済み罠の種類を方向指定で確認する。ステータス、ヘルプ、search、Trap Inspect、自動拾い切替の入口は補助メニューにも残し、A/B/Start/Select + D-pad だけで主要操作を完結させる。地図確認はメインASCII表示に集約するため、補助メニューには置かない。

B短押しメニューは携帯機向け移植UIとして、Rogue 5.4.4 のコマンド処理とは別レイヤーに置く。コマンド定義は `MENU_ACTIONS` に残し、表示配置・初期カーソル・D-pad移動は `rogue_ui.py` の `PAD_ACTION_GRID` で管理する。初期カーソルは誤操作リスクが低く使用頻度も高い `Eat` とし、十字方向に高頻度の `Quaff` / `Read` / `Throw`、上段に積極行動、3段目に装備、下段に `Call` / `Discoveries` / `Drop` の整理・確認系を置く。
...
Rogue commands はプレイ中だけ発火し、専用UIや別ロジックは作らず既存のメニュー項目へのショートカットとして扱う。現行では `t` quick throw、`q` quaff、`r` read、`e` eat、`d` drop、`w` wear、`W` wield、`T` take off、`P` put on ring、`R` remove ring、`z` zap、`i` inventory、`?` help、`s` 周囲 search、`^` trap inspect を実装する。`^` は Rogue 5.4.4 風に方向待ちへ入り、Select+D-pad / Tab+方向 は携帯機向けの即時 Trap Inspect として同じ確認処理へ接続する。メニュー/アイテム選択/方向指定オーバーレイ中はプレイ中 Rogue command を発火させず、`pack.c:get_item()` の `o_packch` 相当として `a-z` を直接選択に使う。直打ち item command は、pack が空でなければ type filter 済み一覧が空でも prompt 相当の overlay へ進み、型違いは `quaff()` / `read_scroll()` / `eat()` / `call()` など各コマンド側で拒否する。無効な letter は `'{item}' is not a valid item` を出して prompt を継続し、pack が空の場合は `you aren't carrying anything` で戻る。Shift+letter は `readchar()` が返す大文字と同じく lowercase `o_packch` に一致しない入力として扱う。ESC / B での item prompt 中断は `pack.c:get_item()` の ESC 分岐と同じくターンを消費せず、直打ち command 由来なら play へ、Pyxel menu 由来なら menu へ戻す。Zap などの方向待ち中断も `misc.c:get_dir()` の ESC 分岐と同じくターン非消費の command abort として扱う。
...
オンライン v3 は Google Sheets + Apps Script へ任意送信する。Logo後、v3 profile未作成ならオンライン登録確認を表示する。NOは local-only profile を保存し、後から Title の Name または Online Ranking で登録できる。オンライン名は小文字英数字のみの `user_name` に統一し、初期値は `rogue54` とする。未登録・local-only・server tokenなしの名前はゲーム内表示だけ `user_name*` とし、正式な保存名・送信名には `*` を含めない。登録済みでserver tokenがある名前は、score未投稿でも正式名として表示する。`rogue54` と dummy 名は client / server の両方で予約名として拒否する。名前入力END後は Apps Script の `checkUser` で users sheet を確認し、既存名なら本人確認として6桁PINで `linkUser`、未使用名なら6桁PIN disclaimer後に `registerUser` へ進む。登録済みprofileで Title の Name に入り、未編集でBを押した場合はキャンセル扱いとする。名前を編集してBを押した場合だけ確認画面を出し、OKなら編集名を local-only profile として保存してTitleへ戻る。Online Ranking から同じ操作をした場合はOnline Rankingへ戻る。users sheet は `user_name`、文字列として扱う平文PIN、server token、`last_sync_at`、PIN失敗回数、lock期限、作成時刻を保持する。server tokenはユーザに表示せず、ローカルprofileでは難読化保存する。PINは暗号化しないため、UI上で再利用しない注意を出す。PIN失敗はserver側で5回失敗・10分lockを持つ。

ゲーム終了時はローカル保存だけを行い、結果画面表示前に通信しない。Title から入ってもゲームオーバー後に入っても、Online Ranking 入場時は通信せず、保存済みGET結果と同期間のlocal bestを合成表示する。UI文言では `sync` / `synced` を避け、ローカル保存は `saved`、サーバ送信は `posted`、ランキングGET更新は `refreshed` と呼び分ける。登録/リンク/切替成功直後と Select で通信し、server時刻基準で24時間に1回だけPOSTを許可する。24時間未満でSelectされた場合はPOSTせずランキングGETだけを行い、完了結果として `Ranking refreshed. / POST once per 24h.` を右上に表示する。POST成功時だけ `last_sync_at` を更新し、24時間未満なら `next_sync_at` とともに cooldown を返す。ただしローカルscoreがまだ無い場合、clientは `syncScoreboard` POSTを出さず、ランキングGETだけを行う。本当の初回score投稿を24時間制限で拒否しないため、server側も `last_sync_at` が空で投稿対象scoreも空のprofile確認syncでは `last_sync_at` を更新しない。通信中boxは操作不可状態を示す用途に限定し、POST可能なら `Syncing scores... / Please wait.`、GET更新のみなら `Refreshing ranking... / Please wait.` を表示する。token不一致時は再リンク導線へ戻す。POSTはローカルbest投稿とprofile送信時刻更新だけを扱い、各スコアの `score_id` で重複行を抑止する。Weekly / Season はGETで取得し、GET結果には同期間のlocal bestも合成して、server反映前でも自分のスコアをランキング内に表示する。GET成功時のonline rankingは期間key付きでローカル保存し、次回入場時やGET失敗時は保存済みrankingとlocal bestを合成する。Local ranking は過去行を通常色にし、死亡直後など現在のresult行だけ `>` と黄色で示す。Weekly / Season は自分の掲載行を黄色にする。スコアボードはトップ10のみを表示し、10位外の結果行は出さない。GET失敗は `Refresh failed.`、POST成功後のGET失敗は `Score posted. / Ranking refresh failed.` として右上に表示する。新しいweek/seasonが空にならないよう、ランキングGET時に今週10件とseason内の過去週dummyをlazy生成する。GETはscore sheet全体を1回だけcontextへ読み、dummy判定とtop10抽出はそのメモリ上で行い、生成dummy行は最後にまとめて書き込む。スコアボード見出しは `2026-W18` / `2026-Spring` の期間keyを直接表示し、POST済みprofileでは19行目に `Posted UTC MM-DD HH:MM / POST after MM-DD HH:MM` を表示する。未POST profileの下部ヒントは操作ヒント行と重複するため表示しない。投稿失敗時はゲーム進行を止めずローカル表示へ戻す。

Online Ranking の通常入場時は、前回通信の右上完了結果を表示しない。右上結果は、その画面で実行した通信操作への一時的な返答として扱う。
...
タイトルBGMは `rogue.py` の `TITLE_BGM_MMLS` で管理する。2026-05-04 時点の起動BGMは `Im - bV - IV - bII` の進行を維持した32小節ループとし、後半16小節で旋律・伴奏を変化させる。
...
`P_POISON` の Strength 低下量は `potions.c:quaff()` と同じく `rnd(3)+1` とし、`RogueRng.rnd()` 経由にする。sustain strength 装備中は `chg_str()` へ進まず、momentary sick 文言だけを出す。

食料は Rogue 5.4.4 `misc.c:eat()` を基準にし、純ロジックを `rogue_food.py` へ小分割する。食べた時に `food_left` を `max(food_left, 0) + HUNGERTIME - 200 + rnd(400)` で増やし、`STOMACHSIZE` で丸め、`hungry_state=0` 相当へ戻す。slime-mold は専用の yummy fruit 文言、ration は `rnd(100)>70` で `s_exp++` と awful 文言、それ以外は good 文言にする。

命名機能と思い出し機能は、potion / scroll / ring / stick の `oi_guess`、weapon / armor 個体単位 `o_label`、discovered list まで実装済み。Call 対象選択は `pack.c:inventory(... CALLABLE)` に合わせて FOOD / AMULET 以外を出し、`command.c:call()` と同じく type-level item が既に `oi_know` なら `misc.c:call_it()` へ進まず `that has already been identified` で拒否する。`things.c:print_disc()` は `oi_know` または `oi_guess` がある potion / scroll / ring / stick を一覧化する。各カテゴリの表示順は `things.c:set_order()` 相当として、発見リストを開く時に `rnd(i)` シャッフルで確定し、描画フレームでは追加の乱数を消費しない。発見リスト用の dummy object は `o_flags=0` なので、ring の補正値や stick のチャージは種類判明済みでも表示しない。`potions.c:quaff()` と `scrolls.c:read_scroll()` の末尾は `misc.c:call_it()` を呼ぶため、使用後に `oi_know` が立った potion / scroll は既存の `oi_guess` を消し、未判明かつ仮名ありなら維持する。

Scroll の正式鑑定も Rogue 5.4.4 `scrolls.c:read_scroll()` の各分岐に合わせる。`S_CONFUSE` / `S_SCARE` / `S_REMOVE` / `S_AGGR` は効果を出すが `scr_info[] .oi_know` を直接立てない。`S_MAP` は `scr_info[S_MAP].oi_know` を立て、隠しドア / 通路 / 罠を実地形として表示対象に戻す。`S_SLEEP` は `no_command` を設定し、`player.t_flags &= ~ISRUN` 相当として Pyxel 側の dash を止める。`S_HOLD` は周囲2マス以内の running monster を実際に `ISHELD` 相当へしたときだけ `scr_info[S_HOLD].oi_know` を立て、対象がいない場合は strange sense of loss で未鑑定のままにする。`S_TELEP` は `teleport()` 後に `cur_room != proom` になった場合だけ識別する。`S_CREATE` は周囲8マスの `step_ok()` 候補から生成し、正式識別は立てない。`S_ARMOR` / `S_ENCH` / `S_PROTECT` は装備中の防具 / 武器へ効果が出ても `scr_info[] .oi_know` を直接立てない。対象装備がない場合、`S_ENCH` / `S_PROTECT` は strange sense of loss を出すが、`S_ARMOR` はメッセージを出さない。

`scrolls.c:read_scroll()` は scroll 効果の分岐前に `leave_pack(obj, FALSE, FALSE)` を一度だけ呼ぶ。Pyxel 版の identify scroll も対象選択の有無にかかわらず使用 scroll stack の消費は1個だけにする。
