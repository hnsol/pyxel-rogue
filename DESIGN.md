...
メッセージは下ログではなく、`message toast` としてダンジョン上に1〜3行だけ一時表示する。新規メッセージ発生時に濃く表示し、一定ターン数で暗くなり、その後消える。フレーム数ではなくターン数基準にし、操作環境や描画フレームレートの差で消える速さが変わりすぎないようにする。表示位置はプレイヤーのいる側を避け、プレイヤーが上寄りなら下、下寄りなら上に出す。流れたログはメッセージ履歴画面で確認できるようにし、toast 自体に検索やフィルタは持たせない。
...
当面は PC / SteamDeck 想定の現行 576×360、80×22 ASCII、右HUD、下ログ、Inventory / Help / Death 画面を磨く。パレット切替と run 表示補助は残してよいが、ゲーム状態を変えない表示設定に限る。
