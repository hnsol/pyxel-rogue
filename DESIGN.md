...
オンライン v3 は Google Sheets + Apps Script へ任意同期する。Logo後、v3 profile未作成ならオンライン登録確認を表示する。NOは local-only profile を保存し、後から Title の Name または Online Ranking で登録できる。オンライン名は小文字英数字のみの `user_name` に統一し、初期値は `rogue54` とする。未登録・local-only・server tokenなしの名前はゲーム内表示だけ `user_name*` とし、正式な保存名・送信名には `*` を含めない。登録済みでserver tokenがある名前は、score未投稿でも正式名として表示する。`rogue54` と dummy 名は client / server の両方で予約名として拒否する。名前入力END後は Apps Script の `checkUser` で users sheet を確認し、既存名なら6桁PINで `linkUser`、未使用名なら6桁PIN disclaimer後に `registerUser` へ進む。登録済みprofileで Title の Name に入り、未編集でBを押した場合はキャンセル扱いとする。名前を編集してBを押した場合だけ確認画面を出し、OKなら編集名を local-only profile として保存する。users sheet は `user_name`、平文PIN、server token、`last_sync_at`、PIN失敗回数、lock期限、作成時刻を保持する。server tokenはユーザに表示せず、ローカルprofileでは難読化保存する。PINは暗号化しないため、UI上で再利用しない注意を出す。PIN失敗はserver側で5回失敗・10分lockを持つ。

ゲーム終了時はローカル保存だけを行い、結果画面表示前に通信しない。Title から入ってもゲームオーバー後に入っても、Online Ranking 入場時は通信せず、保存済みGET結果と同期間のlocal bestを合成表示する。同期は登録/リンク/切替成功直後と Select から実行し、server時刻基準で24時間に1回だけPOSTを許可する。24時間未満でSelectされた場合はPOSTせずランキングGETだけを行い、`Score sync available later. Ranking updated.` と表示する。成功時だけ `last_sync_at` を更新し、24時間未満なら `next_sync_at` とともに cooldown を返す。ただしローカルscoreがまだ無い場合、clientは `syncScoreboard` POSTを出さず、ランキングGETだけを行う。本当の初回score投稿を24時間制限で拒否しないため、server側も `last_sync_at` が空で投稿対象scoreも空のprofile確認syncでは `last_sync_at` を更新しない。同期中は `checking user...`、`registering user...`、`linking user...`、`posting local scores...`、`loading scoreboard...` など進行状態を表示する。token不一致時は再リンク導線へ戻す。同期POSTはローカルbest投稿とprofile同期時刻更新だけを扱い、各スコアの `score_id` で重複行を抑止する。Weekly / Season はGETで取得し、GET結果には同期間のlocal bestも合成して、server反映前でも自分のスコアをランキング内に表示する。GET失敗は投稿成功とは別に表示する。新しいweek/seasonが空にならないよう、ランキングGET時に今週10件とseason内の過去週dummyをlazy生成する。スコアボード見出しは `2026-W18` / `2026-Spring` の期間keyを直接表示し、全タブで `Last sync UTC YYYY-MM-DD HH:MM` を共通表示する。投稿失敗時はゲーム進行を止めずローカル表示へ戻す。
