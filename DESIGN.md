...
オンライン v2 は Google Sheets + Apps Script へ任意同期する。初回オンライン利用時に小文字英数字の `user_id`、表示名、6桁PINを登録し、Apps Script の users sheet に `user_id`、表示名、平文PIN、server token、`last_sync_at` を保存する。Localのみの表示名には描画時だけ `*` を付ける。既存 `user_id` は6桁PINで別端末リンクし、server tokenをローカルに難読化保存する。予約名と dummy 名は client / server の両方で拒否する。PINは暗号化しないため、UI上で再利用しない注意を出す。PIN失敗はserver側で回数制限と待機時間を持つ。

ゲーム終了時はローカル保存だけを行い、結果画面表示前に通信しない。同期は Select から手動実行し、server時刻基準で24時間に1回だけ許可する。成功時だけ `last_sync_at` を更新し、24時間未満なら `next_sync_at` とともに cooldown を返す。同期中は枠付き `syncing...` と進行状態を表示し、成功/失敗/cooldown/認証失敗を表示する。token不一致時は再リンク導線へ戻す。同期時にローカルbestを投稿し、各スコアの `score_id` で重複行を抑止する。投稿失敗時はゲーム進行を止めずローカル表示へ戻す。
