...
`potions.c:quaff()` は効果処理後に `misc.c:call_it(&pot_info[obj->o_which])` を呼ぶ。`call_it()` は `oi_know` が立っていれば既存の `oi_guess` を消し、未判明かつ仮名ありならその仮名を維持する。Pyxel 版では potion 使用後の仮名更新を `rogue_potions.call_it_guess_after_use()` で固定する。
