...
`main.c` の初期 delayed action 登録順は `start_daemon(runners)`, `start_daemon(doctor)`, `fuse(swander, WANDERTIME, AFTER)`, `start_daemon(stomach)` とする。Pyxel 版でも共通スロット順をこれに合わせ、後続の `d_slot()` 相当の再利用順を原作と比較できるようにする。`potions.c:P_LSD` は初回 hallucination で `start_daemon(visuals, BEFORE)` し、`daemons.c:come_down()` で `kill_daemon(visuals)` する。Pyxel 版の実描画は既存の描画層で行うが、delayed action table 上の登録・解除は原作に合わせる。
