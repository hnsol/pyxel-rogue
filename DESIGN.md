...
`a` の again command は Rogue 5.4.4 `command.c:command()` の `last_comm`、`misc.c:get_dir()` の `last_dir`、`pack.c:get_item()` の `last_pick` / `reset_last()` の流れに合わせる。Pyxel 版では keyboard `a` だけを入口とし、補助メニューには置かない。最後のプレイ中 command、方向指定 command の方向、item command の選択 item を記録し、方向 prompt を ESC / B で中断した場合は直前 command を復元する。Select 系や補助メニュー選択は携帯機向け UI として扱い、again の command 入力そのものにはしない。
