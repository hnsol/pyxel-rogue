...
def putNotes(note_len, state, tones, result):
    ...
    for _ in range(loops):
        ...
        if pattern:
            ...
            note_str = list_notes[note % 12] + str(note // 12)
        # Vibrato gating:
        # - tones.json の `vibrato` フィールドは「アタック後、何フレーム経ったらビブラートを開始するか」の遅延値。
        #   0 = ビブラートなし。値が大きいほど遅れて掛かる（例: strings は 90 フレーム遅延）。
        # - pyxel エンジンの 'v' エフェクトは振幅固定なので depth 制御は不可。
        # - Square/Triangle 波は奇数フレームで 'v' を間引いて滑らかに聴かせる。
...
# Pyxel再生データの生成
def compile(src, tones, patterns, slide_marks=None):
    slide_marks = slide_marks or {}
    speed = 240
    note_len = 48
    states = []
    results = []
    ...
    for row, item in enumerate(src):
        ...
        for ch in range(4):
            ...
            prev_eff_len = len(results[ch].get("effect", ""))
            putNotes(note_len, state, tones, results[ch])
            if note is not None and row in slide_marks.get(ch, set()):
                eff = results[ch]["effect"]
                added = len(eff) - prev_eff_len
                if added > 0:
                    results[ch]["effect"] = eff[:prev_eff_len] + ("s" * added)
            state["tick"] += note_len
    sounds = []
    for ch in range(4):
        sound = results[ch]
