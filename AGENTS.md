...
- 参照した原作ファイル名・関数名・定数名は、テスト名、コメント、または `docs/DESIGN.md` から追えるようにする
- `vendor/rogue544/` が見つからない場合だけ、`docs/DESIGN.md` の「元祖ローグのソース参照先」に従って取得方法を確認する
...
- Pyxel 都合の差分は表示・入力・移植UIに限定する。ゲーム状態が変わる差分が必要な場合は、理由と原作との差を `docs/DESIGN.md` に記録してから実装する
...
- README は外向けの入口、`docs/TODO.md` は詳細タスク、`docs/DESIGN.md` は設計判断の記録として役割を分ける
...
- `docs/DESIGN.md` の疑似Markdown階層と role に合っているか確認する
...
- 仕様判断が変わる場合だけ、あわせて `docs/DESIGN.md` / `docs/TODO.md` / `AGENTS.md` へ反映する
...
- 忠実度に関わる実装では、参照した Rogue 5.4.4 のファイル名・関数名・定数名をテスト名、コメント、または `docs/DESIGN.md` から追えるようにする
